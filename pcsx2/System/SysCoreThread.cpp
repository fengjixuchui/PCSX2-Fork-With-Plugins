...
void SysCoreThread::GameStartingInThread()
{
	...
	auto LoadPlugins = (void (*)(uint32_t&, uintptr_t, size_t, uintptr_t, size_t, void*&, int&, int&, bool&, AspectRatioType&))GetProcAddress(GetPCSX2PluginInjector(), "LoadPlugins");
	if (LoadPlugins)
	{
		LoadPlugins(ElfCRC, (uintptr_t)&eeMem->Main, sizeof(eeMem->Main), ElfTextRange.first, ElfTextRange.second, 
			g_gs_window_info.window_handle, g_Conf->GSWindow.WindowSize.x, g_Conf->GSWindow.WindowSize.y, g_Conf->GSWindow.IsFullscreen, g_Conf->EmuOptions.GS.AspectRatio);
	}
#endif
